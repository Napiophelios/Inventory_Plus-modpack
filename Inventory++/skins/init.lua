...
skins.formspec.main = function(name)
	...
	if skins.get_type(skins.skins[name]) == skins.type.MODEL then
		formspec = formspec
			.. "image[5.25,0;1.5,3;"..skins.skins[name].."_preview.png]"
			.. "image[6.75,0;1.5,3;"..skins.skins[name].."_preview_back.png]"

	else
		formspec = formspec
			.. "image[5.25,0;1.5,3;"..skins.skins[name]..".png]"
			.. "image[6.75,0;1.5,3;"..skins.skins[name].."_back.png]"
	end
	local meta = skins.meta[skins.skins[name]]
	if meta then
		if meta.name then
			formspec = formspec .. "label[0,0.75;Name: "..meta.name.."]"
		end
		if meta.author then
			formspec = formspec .. "label[0,1.25;Author: "..meta.author.."]"
		end
		if meta.description then
			formspec = formspec .. "label[0,1.75;"..meta.description.."]"
		end
		if meta.comment then
			formspec = formspec .. 'label[0,2.25;"'..meta.comment..'"]'
		end
	end
local index = 0
	...
	if page > 0 then
		formspec = formspec .. "button[0,8.25;1,.5;skins_page_"..(page-1)..";<<]"
	else
		formspec = formspec .. "button[0,8.25;1,.5;skins_page_"..page..";<<]"
	end
	formspec = formspec .. "button[.75,8.25;6.5,.5;skins_page_"..page..";Page "..(page+1).."/"..math.floor(#skins.list/16+1).."]" -- a button is used so text is centered
	if index > 16 then
		formspec = formspec .. "button[7,8.25;1,.5;skins_page_"..(page+1)..";>>]"
	else
		formspec = formspec .. "button[7,8.25;1,.5;skins_page_"..page..";>>]"
	end
	return formspec
end
...
minetest.register_on_joinplayer(function(player)
	...
	inventory_plus.register_button(player,"skins","Skins")
end)
