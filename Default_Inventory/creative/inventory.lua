...
local refill = minetest.create_detached_inventory("creative_refill", {
	...
	on_put = function(inv, listname, index, stack, player)
		...
		local player_name = player:get_player_name()
		minetest.sound_play("creative_refill",{to_player=player_name, gain = 0.25})
	end,
})
...
local trash = minetest.create_detached_inventory("creative_trash", {
	...
	on_put = function(inv, listname)
		...
		minetest.sound_play("creative_trash", {to_player=player_name, gain = 0.25})
	end,
})
	...
	sfinv.register_page("creative:" .. name, {
		...
		get = function(self, player, context)
			...
			return sfinv.make_formspec(player, context,
				"label[6.55,4.05;" .. minetest.colorize("#FFFF00", tostring(pagenum)) .. " / " .. tostring(pagemax) .. "]" ..
				[[
					image[5.07,3.41;0.8,0.8;creative_refill_icon.png]
					image[4.07,3.42;0.8,0.8;creative_trash_icon.png]
					...
					button[0,3.342;0.8,0.9;creative_clear;X]
					tooltip[creative_search;Search]
					tooltip[creative_clear;Reset]
					listring[current_player;main]
					field_close_on_enter[creative_filter;false]
				]] ..
				"field[1,3.812;2.7,0.5;creative_filter;;" .. minetest.formspec_escape(inv.filter) .. "]" ..
...
creative.register_tab("all", "Creative Inv", minetest.registered_items)
